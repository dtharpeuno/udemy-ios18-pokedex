...
//  DexWidget
//
//  Created by dtharpeuno on 1/5/26.
//
	...
	func getTimeline(in context: Context, completion: @escaping (Timeline<Entry>) -> ()) {
		Task { @MainActor in
			...
			if let results = try? sharedModelContainer.mainContext.fetch(FetchDescriptor<Pokemon>()) {
				...
				let timeline = Timeline(entries: [SimpleEntry.placeholder, SimpleEntry.placeholderTwo], policy: .atEnd)
				completion(timeline)
			}
		}
	}
...
struct SimpleEntry: TimelineEntry {
	...
	static var placeholder: SimpleEntry {
		SimpleEntry(date: .now, name: "bulbasur", types: ["grass", "poison"], sprite: Image(.bulbasaur))
	}
	
	static var placeholderTwo: SimpleEntry {
